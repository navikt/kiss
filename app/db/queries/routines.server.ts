...
export async function updateRoutine(params: {
	...
	return db.transaction(async (tx) => {
		...
		if (!routine) {
			throw new Response(`Fant ikke rutine med id ${params.id}`, { status: 404 })
		}
		...
		await tx.delete(routineOracleRoleCriticalityLinks).where(eq(routineOracleRoleCriticalityLinks.routineId, params.id))
		const orcLinks = params.oracleRoleCriticalities ?? []
		if (orcLinks.length > 0) {
...
export async function getRoutineDeadlinesForSection(sectionId: string): Promise<RoutineDeadlineInfo[]> {
	...
	// Nestet Map (questionId -> answer -> set of applicationIds).
	// Bruker nested Map fremfor en streng-konkatenert nøkkel for å unngå
	// kollisjoner når svartekster kan inneholde skilletegn.
	const answersByQuestion = new Map<string, Map<string, Set<string>>>()
	for (const a of allAnswers) {
		if (a.answer === null) continue
		let byAnswer = answersByQuestion.get(a.questionId)
		if (!byAnswer) {
			byAnswer = new Map<string, Set<string>>()
			answersByQuestion.set(a.questionId, byAnswer)
		}
		let set = byAnswer.get(a.answer)
		if (!set) {
			set = new Set<string>()
			byAnswer.set(a.answer, set)
		}
	...
	for (const [routineId, links] of questionLinksByRoutine) {
		...
		for (const l of links) {
			...
			const matching = answersByQuestion.get(l.questionId)?.get(l.choiceValue)
			if (matching) {
	...
	// Sjekk om noen rutine i det hele tatt krever tech-elementer.
	// Hvis ikke, kan vi hoppe over hele applicationTechnologyElements-spørringen
	// og bare sende videre alle kandidat-apper per rutine.
	const anyRoutineHasTechReq = [...appsByRoutine.keys()].some((rid) => (elemsByRoutine.get(rid)?.length ?? 0) > 0)

	const elementsByApp = new Map<string, Set<string>>()
	if (anyRoutineHasTechReq) {
	...
	// Build results. Sorter apper per rutine etter navn for stabil utskrift.
	const results: RoutineDeadlineInfo[] = []
	for (const routine of sectionRoutines) {
		const apps = (finalAppsByRoutine.get(routine.id) ?? [])
			.slice()
			.sort((a, b) => (appNameById.get(a) ?? "").localeCompare(appNameById.get(b) ?? "", "nb"))
