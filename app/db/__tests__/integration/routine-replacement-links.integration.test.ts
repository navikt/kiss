...
async function copyRoutineOrThrow(routineId: string, performedBy = "test-user") {
...
async function insertCompletedReview(routineId: string, applicationId: string | null, reviewedAt: Date) {
	...
	const r = await db.execute(
		...
		VALUES ('${routineId}', ${appVal}, 'Test Review', '${reviewedAt.toISOString()}', 'completed', 'test-user')
		RETURNING id`,
	)
	return (r.rows[0] as { id: string }).id
}
...
async function insertNeedsFollowUpReview(routineId: string, applicationId: string | null, reviewedAt: Date) {
	...
	const r = await db.execute(
		/* sql */ `INSERT INTO routine_reviews (routine_id, application_id, title, reviewed_at, status, created_by)
		VALUES ('${routineId}', ${appVal}, 'Follow Up Review', '${reviewedAt.toISOString()}', 'needs_follow_up', 'test-user')
		RETURNING id`,
	)
	return (r.rows[0] as { id: string }).id
}
...
async function insertPresetRoutineEffect(controlId: string, routineId: string) {
	...
	const qR = await db.execute(
		...
		VALUES ((SELECT id FROM sections LIMIT 1), 'Test Question', 'boolean', 'test', 'test') RETURNING id`,
	)
...
async function insertScreeningRoutineSelection(appId: string, choiceEffectId: string, routineId: string) {
	...
	const r = await db.execute(
		...
		VALUES ('${appId}', '${choiceEffectId}', '${routineId}', 'test') RETURNING id`,
	)
	return (r.rows[0] as { id: string }).id
}
...
async function insertApplicationControlsWithRoutine(appId: string, controlId: string, routineId: string) {
	...
	const r = await db.execute(
		...
		VALUES ('${appId}', '${controlId}', NULL, 'not_established', 'not_applicable', 0, 0, 0, '{}', ARRAY['${routineId}']::uuid[], false, 'test', 'test') RETURNING id`,
	)
	return (r.rows[0] as { id: string }).id
}
...
describe("routine replacement link propagation", () => {
	...
	beforeEach(async () => {
		...
		`)
		sectionId = (await createSection("Test Section", null, "test-user")).id
	...
	describe('replaceRoutine with deadlinePolicy="reset"', () => {
		it("updates preset_routine_id that existed before replacement", async () => {
			...
			const db = getTestDb()
			await db.execute(
				/* sql */ `UPDATE routines SET status = 'approved', updated_by = 'test' WHERE id = '${routineA.id}'`,
			)
			...
			// Now copy + replace
			const routineB = await copyRoutineOrThrow(routineA.id, "test-user")
			await makeRoutineReady(routineB.id)
			await replaceRoutine(routineB.id, routineA.id, "reset", "test-user")
	...
	describe('replaceRoutine with deadlinePolicy="continue"', () => {
		it("copies latest completed review from A to B with inherited marker", async () => {
			...
			const routineB = await copyRoutineOrThrow(routineA.id, "test-user")
			await makeRoutineReady(routineB.id)
			await replaceRoutine(routineB.id, routineA.id, "continue", "test-user")
	...
	describe("transitive chain A→B→C with continue", () => {
		it("C inherits A's original review date via B's inherited review", async () => {
			...
			// B → C (continue)
			const routineC = await copyRoutineOrThrow(routineB.id, "test-user")
			await makeRoutineReady(routineC.id)
			await replaceRoutine(routineC.id, routineB.id, "continue", "test-user")
		...
		it("mixed chain: A→B (continue) then B→C (reset) — C has no inherited review", async () => {
			...
			const routineC = await copyRoutineOrThrow(routineB.id, "test-user")
			await makeRoutineReady(routineC.id)
			await replaceRoutine(routineC.id, routineB.id, "reset", "test-user")
	...
	describe("migrateExistingReplacementChains", () => {
		...
		async function buildStaleChain(opts: { deadlinePolicy?: "continue" | "reset"; withReviewOnA?: boolean } = {}) {
			...
			await db.execute(
				/* sql */ `UPDATE routines SET status = 'archived', archived_at = NOW(), archived_by = 'test', replaced_by_routine_id = '${routineB.id}' WHERE id = '${routineA.id}'`,
			)
			await db.execute(
				/* sql */ `UPDATE routines SET status = 'approved', updated_by = 'test' WHERE id = '${routineB.id}'`,
			)
		...
		it("handles transitive stale chain A→B→C and migrates all links to C", async () => {
			...
			const routineC = await copyRoutineOrThrow(routineB.id, "test-user")
			await db.execute(
				/* sql */ `UPDATE routines SET status = 'archived', archived_at = NOW(), archived_by = 'test', replaced_by_routine_id = '${routineC.id}' WHERE id = '${routineB.id}'`,
			)
			await db.execute(
				/* sql */ `UPDATE routines SET status = 'approved', updated_by = 'test' WHERE id = '${routineC.id}'`,
			)
			await db.execute(
				/* sql */ `INSERT INTO audit_log (action, entity_type, entity_id, metadata, performed_by, performed_at)
