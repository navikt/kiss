...
describe("screening-sessions", () => {
	...
	describe("completeScreeningSession", () => {
		...
		it("auto-applies preset_routine effects atomically on completion", async () => {
			...
			const naisTeamId = await createNaisTeam(`preset-team-${appId}`, sectionId)
	...
	describe("archiveScreeningSession", () => {
		it("clears stale routine selections when answer changes to one without routine effect", async () => {
			...
			const naisTeamId = await createNaisTeam(`cleanup-team-${appId}`, sectionId)
			await createAppEnvironment(appId, naisTeamId)
			const questionId = await createQuestion("Har dere tilgangsstyring?")
		...
		it("archives a selection when the question is archived outside the current session", async () => {
			...
			const naisTeamId = await createNaisTeam(`archive-team-${appId}`, sectionId)
			await createAppEnvironment(appId, naisTeamId)
