...
export async function handleComplianceIntent(
	...
	appId: string,
	authedUser: NavUser,
	options?: { skipAuthChecks?: boolean; skipSync?: boolean },
	...
	if (intent === "add-persistence") {
		...
		if (!type || !name) {
			return data({
				...
				controlId: "screening",
				screening: true,
				error: `Type og navn er påkrevd (type=${type || "mangler"}, name=${name || "mangler"})`,
			})
		}
		if (!persistenceTypeEnum.includes(type as (typeof persistenceTypeEnum)[number])) {
			return data({ success: false, controlId: "screening", screening: true, error: `Ugyldig persistenstype: ${type}` })
		}
	...
	if (intent === "update-persistence-classification") {
		...
		const [owned] = await db
			...
			.where(and(eq(applicationPersistence.id, persistenceId), eq(applicationPersistence.applicationId, appId)))
		if (!owned) return data({ success: false, controlId: "screening", screening: true })
	...
	if (intent === "selectRoutine") {
		...
		if (!choiceEffectId) throw new Response("Mangler choiceEffectId", { status: 400 })

		// Validate that choiceEffectId belongs to a question relevant to this app
		const { getScreeningDataForApp } = await import("~/db/queries/screening.server")
		const screeningData = await getScreeningDataForApp(appId)
		const allRoutineSelections = screeningData.questions.flatMap((q) => q.choices.flatMap((c) => c.routineSelections))
		const matchingEffect = allRoutineSelections.find((rs) => rs.effectId === choiceEffectId)
		if (!matchingEffect) {
		...
		if (routineId !== null) {
			...
			if (!validRoutineIds.has(routineId)) {
				throw new Response("Ugyldig rutine for denne kontrollen", { status: 400 })
			}
		}
