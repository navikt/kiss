...
describe("entra team sync jobs wrapper", () => {
	...
	it("oppretter jobb med riktig jobType", async () => {
		await createEntraTeamSyncJob("Z990001")
	...
	it("markerer jobb failed og rekaster ved feil i sync-kjøringen", async () => {
		...
		await expect(runTrackedEntraTeamMemberSync({ performedBy: "Z990001" })).rejects.toThrow("boom")
		expect(mockMarkSyncJobFailed).toHaveBeenCalledWith("job-1", "boom", "Z990001", "Synkronisering feilet")
	})
})
