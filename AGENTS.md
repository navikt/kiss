...
12. **`complianceAssessments` og `complianceAssessmentHistory` er DEPRECATED og skal ikke brukes.** Funksjonene `saveAssessment`/`saveAssessmentComment` og filen `applications.deprecated.server.ts` er fjernet (F6). Tabellene består i databasen for historisk audit-trail (markert med `COMMENT ON TABLE ... 'DEPRECATED ...'` i migrasjon `0038`), men ingen produksjonskode skriver til dem. Compliance-status utledes nå fra screening-spørsmål (`screeningQuestions`/`screeningAnswers`), regelsett (`rulesets`/`rulesetControls`), rutiner (`routines`/`routineControls`) og den materialiserte `application_controls`-tabellen. **Ikke gjeninnfør bruk av `complianceAssessments` i ny kode.** Den eneste gjenværende referansen er en defensiv FK-cleanup ved sletting av en applikasjon i `nais.server.ts` for å holde `monitored_applications`-FK-en konsistent.
